...
import top.catnies.firenchantkt.gui.ShowEnchantedBooksMenu
...
class FirShowEnchantedBooksMenu(
    ...
    private fun buildPageItem() {
        previousPageItem?.let {
            previousPageBottom = MenuPageItem(false, it.action) {
                if (gui.page == 0) return@MenuPageItem ItemStack.empty()
        ...
        nextPageItem?.let {
            nextPageBottom = MenuPageItem(true, it.action) {
                if (gui.pageCount == 0) return@MenuPageItem ItemStack.empty() // 总页数为0代表目前没有正在修复的装备
                if (gui.page == gui.pageCount - 1) return@MenuPageItem ItemStack.empty() // 如果当前页数 = (总页数 - 1)就代表是最后一页
